...
//        "password": "SuperS3gura"
    ...
    fun obtenerUsuarios() {
        val pokemonString = """
            {
        "createdAt": 1597728935898,
        "updatedAt": 1597728935898,
        "id": 1,
        "nombre": "Bulbasaur",
        "usuario": 1,
          }
            ...
            .responseString { request, response, result ->
                when (result) {
                    ...
                    is Result.Success -> {
                        val data = result.get()
                        val usuarios = Klaxon()
                            .converter(UsuarioHttp.converterUs).parseArray<UsuarioHttp>(data)
                        if (usuarios != null) {
                            usuarios.forEach {
                                Log.i(
                                    "http-klaxon",
            ...
            .responseString { request, response, result ->
                when (result) {
                    ...
                    is Result.Success -> {
                        val data = result.get()
                        val pokemon = Klaxon()
                            .converter(PokemonHttp.ConverterPk).parseArray<PokemonHttp>(data)
                        if (pokemon != null) {
                            pokemon.forEach {
                                Log.i(
                                    "http-klaxon",
